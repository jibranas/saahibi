...
import { isMorphologyStoreReady } from './lib/morphologyStore.js';
import { isCorpusReady } from './lib/quranCorpus.js';
import { isTranslationStoreReady } from './lib/translations.js';
...
/**
 * A serverless instance starts with no database connection, and every data
 * route guards on `isDbConnected()` and 503s otherwise — so the connection
 * has to be established per request rather than once at boot. `connectDb`
 * caches it, making this a no-op on a warm instance.
 *
 * Connection failures fall through deliberately: routes emit their own 503,
 * and endpoints that need no database (`/api/curriculum`) still answer.
 */
