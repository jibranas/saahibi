...
const router = Router();
const MUZAF_LEMMA_BW = 'Ham';
const ASM_ARABIC = 'حَم';
const ASM_EN = 'father-in-law';
...
function findIdafaAsmaHamPatterns(records) {
  const patterns = [];
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const patterns = findIdafaAsmaHamPatterns(records);
        ...
        beforeInterlude: [
          {
            ...
            <div class="bg-sky-50 p-4 rounded-lg my-4">
              ...
              <div class="bg-purple-50 p-4 rounded-lg mt-4">
                ...
                <p class="text-sm mt-2">الأسماء الخمسة: حَم — رفعًا (حَمُو)، نصبًا (حَمَا)، جرًّا (حَمِي)</p>
              </div>
            </div>
          `,
          },
