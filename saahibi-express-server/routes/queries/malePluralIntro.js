...
function isSoundMalePluralBw(textBw) {
  const bw = String(textBw || '');
  return /uwna$/.test(bw) || /iyna$/.test(bw);
}

function buildMalePluralIntroExamples(records) {
  ...
  for (const record of records) {
    ...
    const isNomForm = /uwna$/.test(bw);
    const isObliqueForm = /iyna$/.test(bw);

    if (
      record.PartOfSpeech === 'N' &&
      Number(record.Person) === 0 &&
      Number(record.Gender) === 1 &&
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const examples = buildMalePluralIntroExamples(records);
