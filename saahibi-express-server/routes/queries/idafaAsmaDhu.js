...
const router = Router();
const MUZAF_LEMMA_BW = '*uw';
const ASM_ARABIC = 'ذُو';
const ASM_EN = 'possessor';
...
function findIdafaAsmaDhuPatterns(records) {
  const patterns = [];
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const patterns = findIdafaAsmaDhuPatterns(records);
        ...
        beforeInterlude: [
          {
            ...
            <div class="bg-sky-50 p-4 rounded-lg my-4">
              ...
              <div class="bg-purple-50 p-4 rounded-lg mt-4">
                ...
                <p class="text-sm mt-2">الأسماء الخمسة: ذُو — رفعًا (ذُو)، نصبًا (ذَا)، جرًّا (ذِي)</p>
              </div>
            </div>
          `,
          },
