...
const router = Router();
const FATHA_YAA_BW_RE = /[aF]Y`?$/;

function buildFathaYaaFemininesExamples(records) {
  ...
  for (const record of records) {
    ...
    if (
      ...
      record.NominalCase &&
      FATHA_YAA_BW_RE.test(String(record.TextBw || '')) &&
  ...
  const wordsWithCases = Array.from(wordCases.entries())
  ...
  return wordsWithCases.flatMap((lemma) => {
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const examples = buildFathaYaaFemininesExamples(records);
