...
const router = Router();
const MUZAF_LEMMA_BW = 'fam';
const ASM_ARABIC = 'فَم';
const ASM_EN = 'mouth';
...
function findIdafaAsmaFamPatterns(records) {
  const patterns = [];
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const patterns = findIdafaAsmaFamPatterns(records);
        ...
        beforeInterlude: [
          {
            ...
            <div class="bg-sky-50 p-4 rounded-lg my-4">
              ...
              <div class="bg-purple-50 p-4 rounded-lg mt-4">
                ...
                <p class="text-sm mt-2">الأسماء الخمسة: فَم — رفعًا (فَو)، نصبًا (فَا)، جرًّا (فِي)</p>
              </div>
            </div>
          `,
          },
