...
function isYaOrAlifMaqsura(c) {
  return c === YA || c === ALIF_MAQSURA;
}

function findIdafaYaShaddaTwoYasPatterns(records) {
  const patterns = [];
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    if (!prevLast || !isYaOrAlifMaqsura(prevLast)) continue;
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const patterns = findIdafaYaShaddaTwoYasPatterns(records);
      ...
      beforeInterlude: [
        {
          type: 'text',
          content: `
          <h3>Idafa — "my" (ي) with shadda (two yās) — ${idx + 1}</h3>
          <div class="bg-sky-50 p-4 rounded-lg my-4">
            <p class="font-bold text-sky-900">مضاف + مضاف إليه (يّ — شدة بسبب ياءين)</p>
            <p class="text-sm mt-2">When the mudhaf ends in <strong>yā</strong> (ي) and the attached pronoun is <strong>yā</strong> (my), the two yās are written as <strong>يّ</strong> (yā with shadda).</p>
            <div class="text-2xl arabic mt-3">${p.combinedText}</div>
            <div class="text-sm text-gray-600 mt-1">${p.combinedTextBw}</div>
            <p class="text-sm mt-2">"${p.muzafLemma}" + يّ (my)</p>
          </div>
        `,
        },
      ],
