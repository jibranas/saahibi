...
const router = Router();
const PRON_TEXT_BW = new Set(['himaA', 'himaA^']);

function findIdafaPronounHimaPatterns(records) {
  const patterns = [];
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const patterns = findIdafaPronounHimaPatterns(records);
      ...
      beforeInterlude: [
        {
          type: 'text',
          content: `
          <h3>مضاف + مضاف إليه (هِمَا — رفع/نصب بعد كسرة أو ياء ساكن) — ${idx + 1}</h3>
          <div class="bg-sky-50 p-4 rounded-lg my-4">
            <p class="font-bold text-sky-900">إضافة: مضاف + ضمير هِمَا (مثنى غائب)</p>
            <p class="text-sm mt-2">When the letter before the pronoun has <strong>كسرة</strong> or <strong>ياء ساكن</strong>, هُمَا becomes <strong>هِمَا</strong> (hima).</p>
            <div class="text-2xl arabic mt-3">${p.combinedText}</div>
            <div class="text-sm text-gray-600 mt-1">${p.combinedTextBw}</div>
            <p class="text-sm mt-2">"${p.muzafLemma}" + هِمَا (their two)</p>
          </div>
        `,
        },
      ],
