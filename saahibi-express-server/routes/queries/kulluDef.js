...
function findKulluDefPatterns(records) {
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const patterns = findKulluDefPatterns(records);
    const limitedPatterns = patterns.slice(0, limit);

    const examples = [];
    limitedPatterns.forEach((pattern, index) => {
      const defType = pattern.mudafIlayhi.hasDET
        ? 'has ال'
        ...
        beforeInterlude: [
          {
            type: 'text',
            content: `
            <h3>كُلّ + Definite Noun — Pattern ${index + 1}</h3>
            <div class="bg-sky-50 p-4 rounded-lg my-4">
              <h4 class="font-bold text-sky-900 text-lg mb-3">كُلّ + معرفة = "All of"</h4>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-blue-50 p-3 rounded">
                  ...
                  <p class="text-blue-700">Meaning: "all of"</p>
                ...
                <div class="bg-amber-50 p-3 rounded">
                  <h5 class="font-bold text-amber-800 mb-2">مضاف إليه (معرفة)</h5>
                  ...
                  <p class="text-xs text-gray-600">State: <strong>Definite (معرفة)</strong> — ${defType}</p>
              ...
              <div class="bg-purple-50 p-4 rounded-lg mt-4">
                ...
                <p class="text-sm text-purple-800 mb-2">"all of ${pattern.mudafIlayhi.lemma || '...'}"</p>
                <p class="text-xs text-gray-600 mt-2">كُلّ + definite noun = <strong>"all of"</strong> — refers to the totality of a known group</p>
              </div>
            </div>
          `,
          },
