...
const router = Router();
const MUZAF_LEMMA_BW = '>ab';
const ASM_ARABIC = 'أَب';
const ASM_EN = 'father';
...
function findIdafaAsmaAbPatterns(records) {
  const patterns = [];
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const patterns = findIdafaAsmaAbPatterns(records);
        ...
        beforeInterlude: [
          {
            ...
            <div class="bg-sky-50 p-4 rounded-lg my-4">
              ...
              <div class="bg-purple-50 p-4 rounded-lg mt-4">
                ...
                <p class="text-sm mt-2">الأسماء الخمسة في الإضافة: رفعًا (أَبُو)، نصبًا (أَبَا)، جرًّا (أَبِي)</p>
              </div>
            </div>
          `,
          },
