...
const router = Router();
const FATHA_HAMZA_DAMMA_BW_RE = /aA\^'?[uaiF&N\[\]]$/;

function buildFathaHamzaDammaExamples(records) {
  ...
  for (const record of records) {
    if (
      ...
      record.NominalCase &&
      FATHA_HAMZA_DAMMA_BW_RE.test(String(record.TextBw || ''))
    ) {
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const examples = buildFathaHamzaDammaExamples(records);
