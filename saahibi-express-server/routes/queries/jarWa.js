...
const router = Router();
function findJarWaPatterns(records) {
  const patterns = [];

  for (let i = 0; i < records.length - 1; i++) {
    const rec = records[i];
    if (rec.PartOfSpeech !== 'P') continue;
    if (String(rec.Text || '').trim() !== 'وَ') continue;
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const patterns = findJarWaPatterns(records);
        ...
        beforeInterlude: [
          {
            type: 'text',
            content: `
            <h3>حرف الجر وَ + اسم مجرور — مثال ${index + 1}</h3>
            <div class="bg-sky-50 p-4 rounded-lg my-4">
              ...
              <div class="bg-purple-50 p-4 rounded-lg mt-4">
                ...
                <p class="text-sm text-purple-800">حرف جر (وَ) متصل بالاسم بعده في كلمة واحدة أحياناً (وَاللَّهِ): وَ = حرف جر، الاسم بعده = مجرور.</p>
              </div>
            </div>
          `,
          },
