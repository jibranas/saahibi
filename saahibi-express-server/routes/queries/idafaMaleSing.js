...
function caseOrder(nominalCase) {
  const c = String(nominalCase || '');
  if (c.includes('NOM')) return 0;
  if (c.includes('ACC')) return 1;
  if (c.includes('GEN')) return 2;
  return 3;
}
...
function isGenderOne(g) {
  return String(g == null ? '' : g).trim() === '1' || Number(g) === 1;
}

function findIdafaMaleSingPatterns(records) {
  const patterns = [];
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    if (!isGenderOne(ilayNoun.Gender)) continue;
  ...
  patterns.sort((a, b) => caseOrder(a.muzaf.nominalCase) - caseOrder(b.muzaf.nominalCase));
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const patterns = findIdafaMaleSingPatterns(records);
