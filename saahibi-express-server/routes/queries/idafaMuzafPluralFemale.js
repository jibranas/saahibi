...
function findIdafaMuzafPluralFemalePatterns(records) {
  const patterns = [];
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const patterns = findIdafaMuzafPluralFemalePatterns(records);
        ...
        beforeInterlude: [
          {
            type: 'text',
            content: `
            <h3>Idafa: مضاف جمع مؤنث — Pattern ${index + 1}</h3>
            <div class="bg-sky-50 p-4 rounded-lg my-4">
              <h4 class="font-bold text-sky-900 text-lg mb-3">Structure: مضاف (جمع مؤنث سالم أو مكسّر) + مضاف إليه</h4>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-green-50 p-3 rounded">
                  <h5 class="font-bold text-green-800 mb-2">المضاف — جمع مؤنث (Muzaf: Fem. Plural)</h5>
                  ...
                  <p class="text-xs text-gray-600">Number: Plural | Gender: Feminine</p>
                </div>
