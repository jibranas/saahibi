...
function findIdafaYaFathaAfterAlifPatterns(records) {
  const patterns = [];
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const patterns = findIdafaYaFathaAfterAlifPatterns(records);
      ...
      beforeInterlude: [
        {
          type: 'text',
          content: `
          <h3>Idafa — "my" (ي) with fatha after alif — ${idx + 1}</h3>
          <div class="bg-sky-50 p-4 rounded-lg my-4">
            <p class="font-bold text-sky-900">مضاف + مضاف إليه (يَ — فتحة بسبب الألف قبلها)</p>
            <p class="text-sm mt-2">When the letter before the attached pronoun ي (my) is <strong>alif</strong> (ا or ى), the ya takes <strong>fatha</strong> (ىَ).</p>
            <div class="text-2xl arabic mt-3">${p.combinedText}</div>
            <div class="text-sm text-gray-600 mt-1">${p.combinedTextBw}</div>
            <p class="text-sm mt-2">"${p.muzafLemma}" + يَ (my)</p>
          </div>
        `,
        },
      ],
