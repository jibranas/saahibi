...
function findIdafaMuzafPluralMaleSaalimPatterns(records) {
  const patterns = [];
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    if (String(record.Gender ?? '') !== '1' || String(record.Number ?? '') !== '3')
      continue;
    if (!muzafCombinedBw.endsWith('iyna') && !muzafCombinedBw.endsWith('uwna'))
      continue;
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const patterns = findIdafaMuzafPluralMaleSaalimPatterns(records);
        ...
        beforeInterlude: [
          {
            type: 'text',
            content: `
            <h3>Idafa: مضاف جمع مذكر سالم — Pattern ${index + 1}</h3>
            <div class="bg-sky-50 p-4 rounded-lg my-4">
              <h4 class="font-bold text-sky-900 text-lg mb-3">Structure: مضاف (جمع مذكر سالم) + مضاف إليه</h4>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-green-50 p-3 rounded">
                  <h5 class="font-bold text-green-800 mb-2">المضاف — جمع مذكر سالم (Muzaf: Sound Masc. Plural)</h5>
                  ...
                  <p class="text-xs text-gray-600">Number: Plural | Gender: Masculine | Sound plural (ـونَ/ـينَ)</p>
                </div>
