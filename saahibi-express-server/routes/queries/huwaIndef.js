...
const router = Router();
function findHuwaIndefExamples(records) {
  const huwaPatterns = [];
  const examples = [];
  ...
  for (let i = 0; i < records.length - 1; i++) {
    ...
    if (currentRecord.Text === 'هُوَ' && currentRecord.TextBw === 'huwa') {
      if (
        nextRecord &&
  ...
  huwaPatterns.forEach((pattern, index) => {
    examples.push({
      surahId: parseInt(String(pattern.huwa.surahId), 10),
      ayahNo: parseInt(String(pattern.huwa.ayahNo), 10),
      words: [
        { wordNo: parseInt(String(pattern.huwa.wordNo), 10) },
      ...
      beforeInterlude: [
        {
          type: 'text',
          content: `
            <h3>Huwa + Indefinite Noun Pattern ${index + 1}</h3>
            <div class="bg-blue-50 p-4 rounded-lg my-4">
              <h4 class="font-bold text-blue-900 text-lg mb-3">Sentence Structure: هُوَ + Indefinite Noun</h4>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-green-50 p-3 rounded">
                  <h5 class="font-bold text-green-800 mb-2">Subject Pronoun</h5>
                  <div class="text-2xl arabic mb-2">${pattern.huwa.text}</div>
                  <div class="text-sm text-gray-600 mb-2">${pattern.huwa.textBw}</div>
                  <p class="text-green-700">Meaning: "He is" / "It is"</p>
                </div>
              ...
              <div class="bg-purple-50 p-4 rounded-lg mt-4">
                ...
                <div class="text-xl arabic mb-2">${pattern.huwa.text} ${pattern.indefinite.text}</div>
                <p class="text-purple-800">Translation: "He/It is ${pattern.indefinite.lemma}"</p>
                <div class="mt-3 text-sm">
                  <p class="font-semibold">Grammar Notes:</p>
                  <ul class="list-disc pl-6 mt-1">
                    <li><strong>هُوَ</strong> is a subject pronoun (3rd person masculine singular)</li>
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const { examples, huwaPatterns } = findHuwaIndefExamples(records);
    ...
    res.json({
      ...
      examples: limited,
      debug: {
        totalHuwaPatterns: huwaPatterns.length,
        samplePatterns: huwaPatterns.slice(0, 5),
      },
    });
  } catch (err) {
    next(err);
