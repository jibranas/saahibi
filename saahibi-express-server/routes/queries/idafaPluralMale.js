...
function findIdafaPluralMalePatterns(records) {
  const patterns = [];
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    if (String(ilayNoun.Gender ?? '') !== '1' || String(ilayNoun.Number ?? '') !== '3')
      continue;
    const ilayBw = String(ilayNoun.TextBw || '');
    if (!ilayBw.endsWith('iyna')) continue;
  ...
  return { patterns, constructCount, idafaCount, dualMaleCount };
}
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const { patterns, constructCount, idafaCount, dualMaleCount } =
      findIdafaPluralMalePatterns(records);
        ...
        beforeInterlude: [
          {
            type: 'text',
            content: `
            <h3>Idafa with Masculine Plural مضاف إليه — Pattern ${index + 1}</h3>
            <div class="bg-sky-50 p-4 rounded-lg my-4">
              <h4 class="font-bold text-sky-900 text-lg mb-3">Structure: مضاف + مضاف إليه (جمع مذكر)</h4>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="bg-amber-50 p-3 rounded">
                  <h5 class="font-bold text-amber-800 mb-2">المضاف إليه — جمع مذكر (Masc. Plural)</h5>
                  ...
                  <p class="text-xs text-gray-600">Number: Plural (جمع)</p>
                  <p class="text-xs text-gray-600">Gender: Masculine (مذكر)</p>
                </div>
              </div>
              <div class="bg-purple-50 p-4 rounded-lg mt-4">
                ...
                <p class="text-purple-800">Translation: "${pattern.muzaf.lemma} of the ${pattern.muzafIlayhi.lemma}"</p>
                <div class="mt-3 text-sm">
                  <p class="font-semibold">Grammar Notes:</p>
                  <ul class="list-disc pl-6 mt-1">
                    <li><strong>${pattern.muzaf.combinedText}</strong> = مضاف in construct state</li>
                    <li><strong>${pattern.muzafIlayhi.combinedText}</strong> = مضاف إليه — <strong>masculine plural (جمع مذكر)</strong>, genitive</li>
                    <li>Sound masculine plurals in genitive/accusative take the <strong>ـينَ (-īna)</strong> ending</li>
                    <li>In nominative, they take <strong>ـونَ (-ūna)</strong>, but in إضافة they are always GEN → <strong>ـينَ (-īna)</strong></li>
