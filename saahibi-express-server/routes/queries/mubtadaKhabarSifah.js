...
function findMubtadaKhabarSifahPatterns(ayahWords) {
  const patterns = [];
  ...
  for (const [, words] of ayahWords.entries()) {
    for (let i = 0; i < words.length - 2; i++) {
      ...
      const state1 = String(n1.NominalState || '').trim();
      const isIndef1 = state1.includes('INDEF');
      const isDef1 = w1.hasDET || !isIndef1;
      ...
      if (g2 === 0 && g3 === 0) continue;
      if (g2 !== 0 && g3 !== 0 && g2 !== g3) continue;

      const num2 = parseInt(String(n2.Number || '0').trim(), 10) || 0;
      const num3 = parseInt(String(n3.Number || '0').trim(), 10) || 0;
      const effNum2 = num2 === 0 ? 1 : num2;
      const effNum3 = num3 === 0 ? 1 : num3;
      if (effNum2 !== effNum3) continue;

      const effectiveGender = g2 !== 0 ? g2 : g3;
      const effectiveNumber = effNum2;

      const mubtadaState = w1.hasDET ? 'Definite (معرفة) - ال' : 'Definite (معرفة)';
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const patterns = findMubtadaKhabarSifahPatterns(ayahWords);
    const limited = patterns.slice(0, limit);

    const wordRefs = [];
    for (const p of limited) {
      wordRefs.push(p.mubtada, p.khabar, p.sifah);
    }
