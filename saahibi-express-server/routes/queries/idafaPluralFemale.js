...
/** Same as source: feminine sound plural GEN ending pattern on Buckwalter TextBw */
const ILAY_FEM_PL_BW = /[A`]t[iK]$/;

function findIdafaPluralFemalePatterns(records) {
  const patterns = [];
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    if (String(ilayNoun.Gender ?? '') !== '2' || String(ilayNoun.Number ?? '') !== '3')
      continue;
    const ilayBw = String(ilayNoun.TextBw || '');
    if (!ILAY_FEM_PL_BW.test(ilayBw)) continue;
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const { patterns, constructCount, idafaCount, matchCount } =
      findIdafaPluralFemalePatterns(records);
        ...
        beforeInterlude: [
          {
            type: 'text',
            content: `
            <h3>Idafa with Feminine Plural مضاف إليه — Pattern ${index + 1}</h3>
            <div class="bg-sky-50 p-4 rounded-lg my-4">
              <h4 class="font-bold text-sky-900 text-lg mb-3">Structure: مضاف + مضاف إليه (جمع مؤنث)</h4>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="bg-amber-50 p-3 rounded">
                  <h5 class="font-bold text-amber-800 mb-2">المضاف إليه — جمع مؤنث (Fem. Plural)</h5>
              ...
              <div class="bg-purple-50 p-4 rounded-lg mt-4">
                ...
                <div class="mt-3 text-sm">
                  ...
                  <ul class="list-disc pl-6 mt-1">
                    ...
                    <li><strong>${pattern.muzafIlayhi.combinedText}</strong> = مضاف إليه — <strong>feminine plural (جمع مؤنث)</strong>, genitive</li>
                    <li>Sound feminine plurals (جمع مؤنث سالم) take <strong>كسرة (-i)</strong> in GEN/ACC</li>
                    <li>The ـاتِ (-āti) ending indicates feminine plural in genitive case</li>
