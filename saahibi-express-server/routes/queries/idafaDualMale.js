...
function findIdafaDualMalePatterns(records) {
  const patterns = [];
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    if (String(ilayNoun.Gender ?? '') !== '1' || String(ilayNoun.Number ?? '') !== '2')
      continue;
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const { patterns, constructCount, idafaCount, dualMaleCount } =
      findIdafaDualMalePatterns(records);
        ...
        beforeInterlude: [
          {
            type: 'text',
            content: `
            <h3>Idafa with Masculine Dual مضاف إليه — Pattern ${index + 1}</h3>
            <div class="bg-sky-50 p-4 rounded-lg my-4">
              <h4 class="font-bold text-sky-900 text-lg mb-3">Structure: مضاف + مضاف إليه (مثنى مذكر)</h4>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="bg-amber-50 p-3 rounded">
                  <h5 class="font-bold text-amber-800 mb-2">المضاف إليه — مثنى مذكر (Masc. Dual)</h5>
              ...
              <div class="bg-purple-50 p-4 rounded-lg mt-4">
                ...
                <div class="mt-3 text-sm">
                  ...
                  <ul class="list-disc pl-6 mt-1">
                    ...
                    <li><strong>${pattern.muzafIlayhi.combinedText}</strong> = مضاف إليه — <strong>masculine dual (مثنى مذكر)</strong>, genitive</li>
                    <li>Dual nouns in genitive/accusative take the <strong>يْنِ (-ayni)</strong> ending</li>
                    <li>In nominative, dual nouns take <strong>انِ (-āni)</strong>, but in إضافة they are always GEN → <strong>يْنِ (-ayni)</strong></li>
