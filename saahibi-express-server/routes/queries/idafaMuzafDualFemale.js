...
function findIdafaMuzafDualFemalePatterns(records) {
  const patterns = [];
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    if (String(record.Gender ?? '') !== '2' || String(record.Number ?? '') !== '2')
      continue;
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const patterns = findIdafaMuzafDualFemalePatterns(records);
        ...
        beforeInterlude: [
          {
            type: 'text',
            content: `
            <h3>Idafa: مضاف مثنى مؤنث — Pattern ${index + 1}</h3>
            <div class="bg-sky-50 p-4 rounded-lg my-4">
              <h4 class="font-bold text-sky-900 text-lg mb-3">Structure: مضاف (مثنى مؤنث) + مضاف إليه</h4>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-green-50 p-3 rounded">
                  <h5 class="font-bold text-green-800 mb-2">المضاف — مثنى مؤنث (Muzaf: Fem. Dual)</h5>
                  ...
                  <p class="text-xs text-gray-600">Number: Dual | Gender: Feminine</p>
                </div>
